...
exports.tree = asyncHandler(async (req, res) => {
  const types = await WineType.findAll();
  const sweetnesses = await WineSweetness.findAll();
  const wines = await Wine.findAll({ include: [{ model: WineDescription, as: 'description' }, { model: WineImage, as: 'images' }] });
