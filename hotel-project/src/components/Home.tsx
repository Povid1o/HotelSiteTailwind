...
const Home = observer(() => {
  ...
  return <main className='v4-home'>
    ...
    <section className='v4-container v4-home__rooms'>
      ...
      <div className='v4-room-grid'>{rooms.map((room: any) => { const price = room.price?.[0]?.price; const image = media(room.images?.[0]); return <article className='v4-card' key={room.id}><div className='v4-card__image'>{image && <img src={image} alt={room.name} />}</div><div className='v4-card__body'><span className='v4-tag'>{room.properties?.[0] || 'Номер'}</span><h3>{room.name}</h3><p>{room.description}</p><div className='v4-card__footer'><strong>{price ? `от ${Number(price).toLocaleString('ru-RU')} ₽` : 'По запросу'}</strong><Link to='/Hotel'>Подробнее →</Link></div></div></article>})}</div>
    </section>
